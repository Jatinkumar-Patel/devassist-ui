...
export async function fetchSnowAttachments(sysId: string) {
  const res = await fetch(`${BRIDGE()}/api/snow/attachments/${encodeURIComponent(sysId)}`);
...
export async function downloadSnowAttachment(attachmentSysId: string): Promise<Blob> {
  const res = await fetch(`${BRIDGE()}/api/snow/attachment/${encodeURIComponent(attachmentSysId)}`);
...
export async function escalateSnowTask(taskSysId: string) {
  const res = await fetch(`${BRIDGE()}/api/snow/escalate/${encodeURIComponent(taskSysId)}`);
