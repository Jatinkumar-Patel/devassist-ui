...
const REGEX_SIGNAL_SEEDS: Array<{ seed: string; regex: RegExp; category: LogHit['category'] }> = [
...
function extractKeywordHitsFromText(text: string, fileName: string): LogHit[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const signal of REGEX_SIGNAL_SEEDS) {
      if (!signal.regex.test(line)) continue;
      hits.push({
        file: fileName,
        line: i + 1,
        text: line.trim().slice(0, 300),
  ...
  for (const seed of GREP_SEEDS) {
    ...
    if (!normalizedSeedText.includes(normalizedSeed)) continue;
...
function extractStackTraces(content: string, fileName: string, maxCount: number): StackTraceSummary[] {
  ...
  for (let i = 0; i < lines.length && traces.length < maxCount; i++) {
    ...
    if (!/(Exception|ERROR|FATAL|SqlException|NullReferenceException|UnauthorizedAccessException|ArgumentException|HttpRequestException|WebException)/i.test(line)) continue;
...
export function buildDiagnosticSummary(result: {
  ...
  let confidence: DiagnosticSummary['confidence'] = 'low';
...
export function buildSuggestions(hits: LogHit[]): CodeSuggestion[] {
